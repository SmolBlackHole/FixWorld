...
namespace FixWorld.Logs
{
	...
	public class Dialog_PublishLogs : Window
	{
		...
		private readonly Dictionary<LogPublisher.PublisherStatus, StatusLabelEntry> statusMessages = new Dictionary<LogPublisher.PublisherStatus, StatusLabelEntry> {
			...
			{LogPublisher.PublisherStatus.Uploading, new StatusLabelEntry("FixWorld_logs_uploading", true)},
			{LogPublisher.PublisherStatus.Shortening, new StatusLabelEntry("FixWorld_logs_shortening", true)},
			{LogPublisher.PublisherStatus.Done, new StatusLabelEntry("FixWorld_logs_uploaded", false)},
			{LogPublisher.PublisherStatus.Error, new StatusLabelEntry("FixWorld_logs_uploadError", false)}
		};
		...
		public Dialog_PublishLogs()
		{
			...
			draggable = true;
			publisher = FixWorldController.Instance.LogUploader;
		}
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			Widgets.Label(titleRect, "FixWorld_logs_publisherTitle".Translate());
			...
			if (publisher.Status == LogPublisher.PublisherStatus.Done)
			{
				...
				if (Widgets.ButtonText(copyBtnRect, "FixWorld_logs_copy".Translate()))
				{
					FixWorldUtility.CopyToClipboard(publisher.ResultUrl);
				}
			}
			var bottomLeftBtnRect = new Rect(inRect.x, inRect.height - ControlButtonSize.y, ControlButtonSize.x, ControlButtonSize.y);
			if (publisher.Status == LogPublisher.PublisherStatus.Error)
			{
				if (Widgets.ButtonText(bottomLeftBtnRect, "FixWorld_logs_retryBtn".Translate()))
				{
			...
			{
				if (Widgets.ButtonText(bottomLeftBtnRect, "FixWorld_logs_browseBtn".Translate()))
				{
			...
			{
				if (Widgets.ButtonText(bottomRightBtnRect, "FixWorld_logs_abortBtn".Translate()))
				{
