...
namespace FixWorld.Logs
{
	public class Dialog_PublishLogsOptions : Window
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			options.UseCustomOptions = !AddOptionCheckbox(l, "FixWorld_logs_useRecommendedSettings", null,
			...
			if (options.UseCustomOptions)
			{
				...
				options.UseUrlShortener = AddOptionCheckbox(l, "FixWorld_logs_shortUrls",
					"FixWorld_logs_shortUrls_tip", options.UseUrlShortener, out _, indent);
				options.IncludePlatformInfo = AddOptionCheckbox(l, "FixWorld_logs_platformInfo",
					"FixWorld_logs_platformInfo_tip", options.IncludePlatformInfo, out _, indent);
				options.AllowUnlimitedLogSize = AddOptionCheckbox(l, "FixWorld_logs_unlimitedLogSize",
					"FixWorld_logs_unlimitedLogSize_tip", options.AllowUnlimitedLogSize, out _, indent);
				options.AuthToken = AddOptionTextField(l, "FixWorld_logs_github_token",
					"FixWorld_logs_github_token_tip", options.AuthToken, indent);
			}
			...
			if (options.UseCustomOptions)
			{
				if (Widgets.ButtonText(rightButtonsRect.LeftPartPixels(buttonSize.x), "FixWorld_logs_toClipboardBtn".Translate()))
				{
					Close();
					OnCopy?.Invoke();
				}
			}
			if (Widgets.ButtonText(rightButtonsRect.RightPartPixels(buttonSize.x), "FixWorld_logs_uploadBtn".Translate()))
			{
				OnAcceptKeyPressed();
			}
