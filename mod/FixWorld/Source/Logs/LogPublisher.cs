...
namespace FixWorld.Logs
{
    ...
    {
        private const string RequestUserAgent = "FixWorld_log_uploader";
        ...
        private const string GistDescription = "Rimworld output log published using FixWorld";
        ...
        private const bool DenyPublicUpload =
            ...
            false;
#endif
        private readonly string GitHubAuthToken = "RuEvo2u9gsaCeKA9Bamh4sa57FOikUYkHhLH_phg".Reverse().Join(""); // GitHub will revoke any tokens committed
        ...
        public void ShowPublishPrompt()
        {
            if (PublisherIsReady())
            {
                ...
                Find.WindowStack.Add(new Dialog_PublishLogsOptions(
                    "FixWorld_logs_shareConfirmTitle".Translate(),
                    "FixWorld_logs_shareConfirmMessage".Translate(),
                    optionsHandle.Value
                )
                {
        ...
        public void BeginUpload()
        {
...
#if TEST_MOCK_UPLOAD
			FixWorldController.Logger.Message(collatedData);
			FixWorldUtility.CopyToClipboard(collatedData);
			MockUpload();
#else
            ...
            Action<Exception> onRequestFailed = ex =>
            {
                ...
                FixWorldController.Logger.Warning("Exception during log publishing (gist creation): " + ex);
            };
            try
            {
                ...
                FixWorldUtility.AwaitUnityWebResponse(activeRequest, OnUploadComplete, onRequestFailed,
        ...
        {
            UpdateCustomOptionsUsage();
            FixWorldUtility.CopyToClipboard(PrepareLogData());
        }
        ...
        private void OnRequestError(string errorMessage)
        {
            ...
            FinalizeUpload(false);
            FixWorldController.Logger.Error(errorMessage + "\n" + Environment.StackTrace);
        }
        ...
        private void BeginUrlShortening()
        {
            ...
            Action<Exception> onRequestFailed = ex =>
            {
                if (userAborted) return;
                FinalizeUpload(true);
                FixWorldController.Logger.Warning("Exception during log publishing (url shortening): " + ex);
            };
            try
            {
                ...
                activeRequest.SetRequestHeader("User-Agent", RequestUserAgent);
                FixWorldUtility.AwaitUnityWebResponse(activeRequest, OnUrlShorteningComplete, onRequestFailed, HttpStatusCode.Created);
            }
            catch (Exception e)
            {
                onRequestFailed(e);
        ...
        {
            var filePath = FixWorldUtility.TryGetLogFilePath();
        ...
        {
            var harmonyInstance = FixWorldController.Instance.HarmonyInst;
        ...
        internal static void RegisterSettings(ModSettingsPack pack)
        {
            optionsHandle = pack.GetHandle<LogPublisherOptions>("logPublisherSettings",
                "FixWorld_setting_logPublisherSettings_label".Translate(), null);
