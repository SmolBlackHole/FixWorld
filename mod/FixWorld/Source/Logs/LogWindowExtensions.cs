...
namespace FixWorld.Logs
{
	...
	public static class LogWindowExtensions
	{
		...
		internal static void PrepareReflection()
		{
			...
			if (selectedMessageField == null) FixWorldController.Logger.Error("Failed to reflect EditWindow_Log.selectedMessage");
		...
		internal static void DrawLogWindowExtensions(Window window, Rect inRect)
		{
			...
			for (int i = 0; i < widgets.Count; i++)
			{
				...
				{
					FixWorldController.Logger.Error("Exception while drawing log window widget: " + e);
					widgets.RemoveAt(i);
					break;
				}
			}
		...
		{
			FixWorldUtility.CopyToClipboard(logMessage.text + "\n" + logMessage.StackTrace);
		}
		...
		private static void AddOwnWidgets()
		{
			...
			AddLogWindowWidget((window, area, message, row) =>
			{
				...
				if (row.ButtonText("FixWorld_logs_shareBtn".Translate()))
				{
			...
			AddLogWindowWidget((window, area, message, row) =>
			{
				if (row.ButtonText("FixWorld_logs_filesBtn".Translate()))
				{
					Find.WindowStack.Add(new FloatMenu(new List<FloatMenuOption> {
					new FloatMenuOption("FixWorld_logs_openLogFile".Translate(), () => {
						ShellOpenLog.Execute();
					}),
					new FloatMenuOption("FixWorld_logs_openSaveDir".Translate(), () => {
						ShellOpenDirectory.Execute(GenFilePaths.SaveDataFolderPath);
					}),
					new FloatMenuOption("FixWorld_logs_openModsDir".Translate(), () => {
			...
			AddLogWindowWidget((window, area, message, row) =>
			{
				if (message != null)
				{
					if (row.ButtonText("FixWorld_logs_copy".Translate()))
					{
						CopyMessage(message);
					}
				}
