...
{
    /// <summary>
    /// The base class for all mods using FixWorld library. All classes extending ModBase will be instantiated
    /// automatically by <see cref="FixWorldController"/> at game initialization.
    ...
    {
        public const string HarmonyInstancePrefix = "FixWorld.";
        ...
        protected ModBase()
        {
            ...
            if (!string.IsNullOrEmpty(settingsPackId))
            {
                ...
                {
                    Settings = FixWorldController.Instance.Settings.GetModSettings(settingsPackId, modContentPackInt?.Name);
                }
                else
                {
        ...
        internal void ApplyHarmonyPatches()
        {
            ...
            if (HarmonyAutoPatch)
            {
                ...
                if (harmonyId == null)
                {
                    harmonyId = $"FixWorld.{LogIdentifierSafe}";
                    GetLogger().Warning($"Failed to identify PackageId, using \"{harmonyId}\" as Harmony id instead.");
                }
                try
                {
                    if (FixWorldController.Instance.ShouldHarmonyAutoPatch(GetType().Assembly, harmonyId))
                    {
        ...
        /// <summary>
        /// Called during FixWorld <see cref="Mod"/> instantiation, accounting for mod load order.
        ...
        /// <summary>
        /// Called when FixWorld receives the <see cref="StaticConstructorOnStartup"/> call.
        /// Load order among mods implementing <see cref="ModBase"/> is respected.
        /// Called after the static constructors for non-FixWorld mods have executed. Is not called again on def reload
        /// </summary>
