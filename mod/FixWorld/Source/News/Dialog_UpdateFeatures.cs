...
namespace FixWorld.News
{
    ...
    public class Dialog_UpdateFeatures : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            Widgets.Label(titleRect, "FixWorld_features_title".Translate());
        ...
        private void DrawEntryTitle(FeatureEntry entry, float width, ref float curY, bool skipDrawing)
        {
            if (!skipDrawing)
            {
                ...
                Text.Font = GameFont.Medium;
                var titleText = entry.def.titleOverride ?? "FixWorld_features_update".Translate(entry.def.modNameReadable, entry.def.assemblyVersion);
        ...
        protected float DrawEntryLinkWidget(Rect titleRect, UpdateFeatureDef forDef)
        {
            if (forDef.linkUrl != null)
            {
                ...
                Widgets.Label(linkRect, "FixWorld_features_link".Translate());
                GUI.color = prevColor;
                GenUI.ResetLabelAlign();
                ...
                if (Mouse.IsOver(linkRect))
                {
                    ...
                    var descriptionText = "FixWorld_features_linkDesc".Translate();
        ...
        private void DoIgnoreNewsProviderToggle(Vector2 togglePos, FeatureEntry entry)
        {
            ...
            if (wasOn != isOn)
            {
                ...
                if (isOn || FixWorldUtility.ShiftIsHeld)
                {
                    ToggleIgnoredState();
                }
                else
                {
                    Find.WindowStack.Add(new Utils.Dialog_Confirm(
                        "FixWorld_features_confirmIgnore".Translate(entry.def.modNameReadable),
                        ToggleIgnoredState,
                        false,
                        "FixWorld_features_confirmIgnoreTitle".Translate())
                    );
                }
            }
        ...
        private List<DescriptionSegment> ParseEntryContent(string content, bool trimWhitespace, out IEnumerable<string> requiredImages)
        {
            ...
            try
            {
                ...
                foreach (var contentSegmentString in segmentStrings)
                {
                    ...
                    else if (segmentString.StartsWith(CaptionSegmentTag))
                    {
                        ...
                        {
                            FixWorldController.Logger.Warning("Improperly formatted feature content. Caption must follow img. Content:" + content);
                            continue;
                        }
            ...
            {
                FixWorldController.Logger.Warning("Failed to parse UpdateFeatureDef content: {0} \n Exception was: {1}", content, e);
        ...
        private static float GetLinkTextWidth()
        {
            ...
            Text.Font = GameFont.Small;
            var width = Text.CalcSize("FixWorld_features_link".Translate()).x;
            Text.Font = prevFont;
            return width;
        }
