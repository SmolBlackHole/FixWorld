	...
	public class UpdateFeatureManager : PersistentDataManager, IUpdateFeaturesDevActions
	{
		...
		internal void OnEarlyInitialize()
		{
			...
			void ResolveAndInjectNewsDefs()
			{
				...
				{
					FixWorldController.Logger.Error("Failed to load UpdateFeatureDefs: " + e);
				}
			}
		}
			...
			if (ShowNewsSetting.Value || manuallyOpened)
			{
				...
				else
				{
					...
					var filteredByAudience = FilterFeatureDefsByMatchingAudience(filteredByVersion,
						FixWorldController.Instance.ModSpotter.FirstTimeSeen,
						e => FixWorldController.Logger.ReportException(e)
					);
				...
				if (defsToShow.Count > 0)
				{
					...
					var newsDialog = manuallyOpened
						? new Dialog_UpdateFeaturesFiltered(defsToShow, IgnoredNewsProvidersSetting,
							this, FixWorldController.Instance.ModSpotter)
		...
		{
			ShowNewsSetting = pack.GetHandle("modUpdateNews", "FixWorld_setting_showNews_label".Translate(), "FixWorld_setting_showNews_desc".Translate(), true);
			var allNewsHandle = pack.GetHandle("showAllNews", "FixWorld_setting_allNews_label".Translate(), "FixWorld_setting_allNews_desc".Translate(), false);
			allNewsHandle.Unsaved = true;
			allNewsHandle.CustomDrawer = rect =>
			{
				if (Widgets.ButtonText(rect, "FixWorld_setting_allNews_button".Translate()))
				{
					if (!TryShowDialog(true))
					{
						Find.WindowStack.Add(new Dialog_MessageBox("FixWorld_setting_allNews_fail".Translate()));
					}
				}
				return false;
			};
			...
			var ignored = pack.GetHandle<IgnoredNewsIds>("ignoredUpdateNews",
				"FixWorld_setting_ignoredUpdateNews_label".Translate(), null);
