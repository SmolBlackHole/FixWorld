...
namespace FixWorld.Patches
{
	...
	{
		[HarmonyPostfix]
		public static void InjectFixWorldEntries(Dialog_Options __instance)
		{
			OptionsDialogExtensions.InjectFixWorldModEntries(__instance);
		}
	}
	...
	internal class DialogOptions_DoModOptions_Patch
	{
		...
		{
			if (!patched)
				FixWorldController.Logger.Error(nameof(DialogOptions_DoModOptions_Patch) + " could not be applied.");
		}
		...
		public static IEnumerable<CodeInstruction> InterceptWindowOpening(IEnumerable<CodeInstruction> instructions)
		{
			...
			foreach (var inst in instructions)
			{
				...
				{
					// replace vanilla Dialog_ModSettings construction with our own for FixWorld mod entries
