...
namespace FixWorld.Settings
{
    ...
    public static class SettingHandleConvertibleUtility
    {
        ...
        private static void CopySerializedMembersFromObject(object source, object destination)
        {
            ...
            // fields
            foreach (var field in source.GetType().GetFields(FixWorldUtility.AllBindingFlags))
            {
            ...
            // properties
            foreach (var prop in source.GetType().GetProperties(FixWorldUtility.AllBindingFlags))
            {
