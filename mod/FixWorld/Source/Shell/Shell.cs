...
namespace FixWorld.Shell
{
    ...
    public static class Shell
    {
        ...
        public static bool StartProcess(ProcessStartInfo psi, ref Process process)
        {
            ...
            if (psi == null)
            {
                FixWorldController.Logger.ReportException(new Exception("Could not start process, ProcessStartInfo is empty"));
                return false;
            }
            try
            {
            ...
            catch (Win32Exception e)
            {
                if (psi.FileName.StartsWith("\"") &&
                    ...
                    FixWorldController.Logger.Warning("Detected surround quotes on shell filename, " +
