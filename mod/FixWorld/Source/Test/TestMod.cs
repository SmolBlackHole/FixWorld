...
namespace FixWorld.Test {
	...
	public class TestMod : ModBase {
		...
		public TestMod() {
			Instance = this;
			Settings.EntryName = "FixWorldTestMod";
		}
		...
		private void TestDoLaterScheduler() {
			var doLater = FixWorldController.Instance.DoLater;
			...
			everyFrame = () => {
				if (numCalls < 60) {
					FixWorldController.Instance.DoLater.DoNextUpdate(everyFrame);
					numCalls++;
			...
			everyMapLoaded = map => {
				FixWorldController.Instance.DoLater.DoNextMapLoaded(everyMapLoaded);
...
#pragma warning disable 649 // unassigned field
		...
		private static void TestExternalSettingsPack() {
			var pack = FixWorldController.SettingsManager.GetModSettings("externalTestMod", "External Test Mod");
