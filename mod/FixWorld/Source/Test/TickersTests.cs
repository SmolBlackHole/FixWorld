...
namespace FixWorld.Test {
	...
	public class TickingTestThing : ThingWithComps {
		...
		public override void SpawnSetup(Map map, bool respawningAfterLoad) {
			...
			FixWorldController.Instance.DistributedTicker.RegisterTickability(TickMethod, tickInterval, this);
		}
		...
		public override IEnumerable<Gizmo> GetGizmos() {
			yield return new Command_Action {
				...
				action = () => {
					var map = Map;
					FixWorldController.Instance.TickDelayScheduler.ScheduleCallback(() => GenSpawn.Spawn(this, Position, map), 5 * 60);
					DeSpawn();
				}
			};
			...
			yield return new Command_Action {
				...
				action = () => FixWorldController.Instance.TickDelayScheduler.ScheduleCallback(MessageCallback, 3 * 60, this)
			};

			yield return new Command_Action {
				defaultLabel = "Recurring callback",
				action = () => FixWorldController.Instance.TickDelayScheduler.ScheduleCallback(MessageCallback, 3 * 60, this, true)
			};

			yield return new Command_Action {
				defaultLabel = "Clear callback",
				action = () => FixWorldController.Instance.TickDelayScheduler.TryUnscheduleCallback(MessageCallback)
			};
		}
