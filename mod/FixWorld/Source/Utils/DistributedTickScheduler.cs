...
namespace FixWorld.Utils
{
    ...
    public class DistributedTickScheduler
    {
        ...
        public void RegisterTickability(Action callback, int tickInterval, Thing owner)
        {
            ...
            {
                FixWorldController.Logger.Warning("DistributedTickScheduler tickability already registered for: " + owner);
            }
            else
            {
            ...
            public void Tick(int currentTick)
            {
                ...
                for (; currentIndex < maxIndex; currentIndex++)
                {
                    ...
                    if (entry.owner.Spawned)
                    {
                        ...
                        {
                            FixWorldController.Logger.Error("DistributedTickScheduler caught an exception while calling {0} registered by {1}: {2}",
                                FixWorldUtility.DescribeDelegate(entry.callback), entry.owner, e);
                        }
                    }
                    else
                    {
