    ...
    public class TickDelayScheduler
    {
        ...
        private bool DoCallback(SchedulerEntry entry)
        {
            if (entry.owner == null || entry.owner.Spawned)
            {
                ...
                {
                    FixWorldController.Logger.Error("TickDelayScheduler caught an exception while calling {0} registered by {1}: {2}",
                        FixWorldUtility.DescribeDelegate(entry.callback), entry.owner == null ? "[null]" : entry.owner.ToString(), e);
                }
            }
            return false;
        }
