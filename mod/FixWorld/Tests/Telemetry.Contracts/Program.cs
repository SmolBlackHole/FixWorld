...
internal static class Program
{
    ...
    private static void Profiles()
    {
        ...
        using var bench = new Profiler<int>();
        var probe = bench.GetSlot(0);
        using (var failed = probe.Measure())
        { failed.Fail(); }
    ...
    private static void Publication()
    {
        ...
        using var json = new StringWriter();
        diagnostics.Store.WriteJson(json);
    ...
    {
        var scheduler = new DistributedTickScheduler();
        scheduler.Initialize(0);
        var owner = new Verse.Thing();
        int calls = 0;
        scheduler.RegisterTickability(() => calls++, 1, owner);
        scheduler.Tick(1);
